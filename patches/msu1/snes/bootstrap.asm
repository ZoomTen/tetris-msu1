...
Interrupt_MSU1:
	...
	lda   f_fading
	bit.b #%00000010
	bne   .fading_jmp          ; if we are fading out, jump to fade routine
...
.process_audio:
	...
	bit.b #%00000100
	bne   .pause_song	; if bit 2 set, pause the song

	bit.b #%00001000
	bne   .resume_song	; if bit 3 set, resume the song
...
; otherwise, set fading flags
	...
	jmp   .reset_restart_flag	; end here

.stop_music:
	bit   !MSU_STATUS
	bvs   .jmp_skip		; v flag = bit 6
...
.pause_song:
	...
	lda.b #%11111110
	and   !MSU_CONTROL	; pause MSU playback
	sta   !MSU_CONTROL	; write back to the control register

	bra .reset_restart_flag
...
; check version
	lda   !MSU_STATUS
	and.b #%000000111
	cmp.b #2			; is MSU version 2?
	bcc   .restart_song	; if < 2 then resume unsupported; restart song instead

	lda.b #%00000100
	ora   i_play_mode	; add the resume bit to the play mode
	sta   !MSU_CONTROL	; write back to the control register
